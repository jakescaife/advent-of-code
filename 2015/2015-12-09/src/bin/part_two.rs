...
fn main() {
    ...
    println!("AOC 2015-09 Part Two: {}", puzzle_solution);
}

fn solve_puzzle(input: &str) -> u32 {
    let mut journey = Journey::from(input);
    let mut maximum_distance = journey.total_distance();

    while journey.next_permutation().is_some() {
        maximum_distance = maximum_distance.max(journey.total_distance());
    }
...
mod tests {
    ...
    fn example_solution() {
        ...
        assert_eq!(982, solve_puzzle(data));
    }
}
