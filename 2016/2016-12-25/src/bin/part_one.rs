...
fn main() {
    ...
    println!("AOC 2016-25 Part One: {}", solve_puzzle(&input));
}

fn solve_puzzle(input: &str) -> i32 {
    for initial in 0.. {
        let mut computer = Computer::default();
        *computer.memory.register('a') = initial;

        if computer.execute_instructions(input) {
            return initial;
        }
    }
...
impl Computer {
    fn execute_instructions(&mut self, instructions: &str) -> bool {
        ...
        outputs.chunks(2).all(|x| x[0] == 0 && x[1] == 1)
    }
